#!/usr/bin/env python3
"""
Parallel TTS Server Entry Point

Uses vLLM batch processing for parallel T3 generation + concurrent S3Gen.
This achieves much better throughput than sequential token streaming.
...
if __name__ == "__main__":
    ...
    from core.parallel_synthesizer import ParallelSynthesizer
    ...
    class ParallelTTSService:
        """Parallel TTS service using vLLM batch processing."""

        def __init__(self, device_index=0, s3gen_workers=2, diffusion_steps=3, max_connections=50):
            self.device = "cuda"
            ...
            logger.info("parallel_tts_service_initialized", device=f"cuda:{device_index}")

        def load_model_sync(self):
            logger.info("loading_parallel_tts_model")
            self.synthesizer.load_sync()
            logger.info("model_loaded")

        async def handle_connection(self, websocket: WebSocket, conn_id: str):
            """Handle WebSocket connection with parallel streaming audio."""
    ...
    # Create FastAPI app
    app = FastAPI(title="Parallel TTS Service", version="0.3.0")
    service = None
    ...
    async def startup():
        ...
        logger.info("parallel_tts_service_ready")
    ...
    async def health_check():
        ...
        return {
            ...
            "s3gen_workers": service.s3gen_workers,
            "diffusion_steps": service.diffusion_steps,
            "gpu": gpu_info,
    ...
    s3gen_workers = int(os.getenv("TTS_S3GEN_WORKERS", "2"))
    diffusion_steps = int(os.getenv("TTS_DIFFUSION_STEPS", "3"))

    logger.info(
        "starting_parallel_tts_server",
        port=port,
